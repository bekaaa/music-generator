...
class Input(object):
	...
	def new_batch(self):
		...
		self.x.reshape(self.batch_size, self.num_steps, self.hidden_size)
		# reshape Y to 2 dimesions
		self.y.reshape(self.num_all_elem, self.element_size)
...
def Main(input_):
	...
	for i in range(input_.num_loops_in_epoch):
		x, y = inpt_.next_batch()
		feed_dict = { tf_x : x, tf_y : y }
		cost, _, state = sess.run([cost, optimizer, state], feed_dict=feed_dict)
		print('Epoch {}, step {}, loss {:.3f}'.format(0, i, cost))
	#---------------------------------------------------------------------------
